...
use crate::models::*;
use crate::state::{MarketState, PositionMap, SymbolMetaMap};
use crate::strategy;
...
pub async fn bootstrap_state(
    ...
    for symbol in symbols {
        ...
        if let Ok(klines) = client.fetch_klines(symbol, "15m", 672).await {
            for k in &klines {
                if let Some(vol_str) = k.get(5).and_then(|v| v.as_str()) {
...
pub async fn run_kline_stream(
    ...
    loop {
        ...
        match connect_kline_ws(&config, &symbols, &market_state, &positions, &meta_map, &db_tx, &client).await {
    ...
    while let Some(msg) = read.next().await {
        ...
        if let Some(mut entry) = market_state.get_mut(symbol) {
            ...
            if kd.interval == "1d" {
                ...
                // 15m candle closed → update rolling average.
                if kd.is_closed {
        ...
        if let Some(sig) = signal {
            info!(
                "🚀 BREAKOUT SIGNAL: {} | price={:.6} > prev_high={:.6} | vol={:.2} > 3×avg={:.2}",
                sig.symbol, sig.price, sig.previous_day_high, sig.volume_15m, sig.avg_volume_7d
            );
            ...
            tokio::spawn(async move {
                if let Err(e) = execute_signal(&client, &config, &positions, &meta_map, &db_tx, &sig).await {
...
async fn execute_signal(
    ...
    // Double-check no position exists (race guard).
